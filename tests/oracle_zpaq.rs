...
fn zpaq_cdchunking_cuts() {
    ...
    let m: Vec<usize> = cdchunking::Chunker::new(cdchunking::ZPAQ::new(13)).slices(&buf).map(|v| v.len()).collect();
    assert_eq!(
        &m[..],
...
mod oracle_zpaq {
    ...
    #[test]
    fn t1() {
        c(&[25, 5, 82, 84, 53, 94, 27, 24, 98, 47, 7, 7, 6, 34, 60, 98, 20, 64, 17, 5, 62, 40, 94, 79, 33, 1, 0])
    }

    #[test]
    fn t2() {
        c(&[25, 5, 82, 84, 53, 94, 27, 24, 98, 47, 7, 7, 6, 34, 60, 98, 20, 64, 17, 5, 62, 40, 94, 79, 33, 1])
    }

    #[test]
    fn t3() {
        c(&[25, 5, 82, 84, 53, 94, 27, 24, 98, 47, 7, 7, 6, 34, 60, 98, 20, 64, 17, 5, 62, 40, 94, 79, 33, 0, 0])
    }
}
